...
function Swipe(container, options) {
  ...
  var browser = {
    ...
    touch: ('ontouchstart' in window) || isIE || window.DocumentTouch && document instanceof DocumentTouch,
  ...
  function setup() {
    ...
    if(options.pair && !modified) {
      ...
      for (var i = 0; i < ell; i++) {
        ...
        var j = -1;
        //get starting point for extra div insert
        if(i + 1 == ell && ell % options.pair != 0) { // last elements wrapper
          j = ell - ell % options.pair;
        }
        else if(i > 0 && (i + 1) % options.pair == 0) {
          j = i + 1 - options.pair;
        }
    ...
    if(options.callback) options.callback.call(null, getIndex());
  }
  ...
  var events = {
    ...
    start: function(event) {
      ...
      if(!isIE) touches = event.touches[0];
      else touches = event;
  ...
  return {
    ...
    kill: function() {
      ...
      while(pos--) {
        ...
        if(twoIs && length == 4 && pos > length - 3) {
